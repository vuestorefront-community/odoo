    ...
    <SfMegaMenu :visible="isSearchOpen" :title="$t('Search results')" class="search">
      <transition name="sf-fade" mode="out-in">
        <div v-if="products && products.length > 0" key="results" class="search__wrapper-results">
          <SfMegaMenuColumn :title="$t('Categories')" class="
            ...
            <SfList v-if="categories.length">
              <SfListItem v-for="(category, key) in categories" :key="key">
                <SfMenuItem :label="category.label" :link="uiHelper.getCatLinkForSearch(category)">
                  <template #mobile-nav-icon> &#8203; </template>
                </SfMenuItem>
              </SfListItem>
            </SfList>
          </SfMegaMenuColumn>
          <SfMegaMenuColumn :title="$t('Product suggestions')"
            ...
            <SfScrollable class="results--desktop desktop-only" show-text="" hide-text="">
              <div class="results-listing">
                <SfProductCard v-for="(product, index) in products" :key="index" class="result-card" :regular-price="
                  $n(productGetters.getPrice(product).regular, 'currency')
                " :imageWidth="216" :imageHeight="288" :nuxtImgConfig="{ fit: 'cover' }" image-tag="nuxt-img"
                  :score-rating="productGetters.getAverageRating(product)" :reviews-count="7"
                  :image="$image(productGetters.getCoverImage(product), 216, 288, productGetters.getImageFilename(product))"
                  :alt="productGetters.getName(product)" :title="productGetters.getName(product)"
                  :link="localePath(goToProduct(product))" @click:wishlist="addItemToWishlist({ product })"
                  @click="$emit('close')" />
              </div>
            </SfScrollable>
            <div class="results--mobile smartphone-only">
              <SfProductCard v-for="(product, index) in products" :key="index" class="result-card" :imageWidth="216"
                :imageHeight="288" :nuxtImgConfig="{ fit: 'cover' }" image-tag="nuxt-img" :regular-price="
                  $n(productGetters.getPrice(product).regular, 'currency')
                " :score-rating="productGetters.getAverageRating(product)" :reviews-count="7"
                :image="$image(productGetters.getCoverImage(product), 216, 288, productGetters.getImageFilename(product))"
                :alt="productGetters.getName(product)" :title="productGetters.getName(product)"
                :link="localePath(goToProduct(product))" @click="$emit('close')" />
            </div>
          </SfMegaMenuColumn>
